...
tools: Read, Write, Edit, Grep, Glob, Bash, WebSearch, WebFetch, TodoWrite, mcp__claude-in-chrome__tabs_context_mcp, mcp__claude-in-chrome__tabs_create_mcp, mcp__claude-in-chrome__navigate, mcp__claude-in-chrome__computer, mcp__claude-in-chrome__javascript_tool, mcp__claude-in-chrome__read_console_messages, mcp__claude-in-chrome__resize_window
model: sonnet
---
...
## Project context — Portfolio Dashboard (overrides the above)

This project's `CLAUDE.md` is authoritative and outranks any generic guidance in
this file. Read it before acting. In particular:

- **Stack is vanilla HTML + CSS + ES modules**, served by `dashboard/serve.py`.
  It is **not** React, TypeScript, Tailwind, or any build tool. Ignore any
  instruction above that mandates those.
- Live GBP investment dashboard. Design **desktop-first and dense**; the Overview
  is viewport-locked to `100vh` and must not scroll.
- `Design.pdf` at the project root is the binding visual reference. Match it;
  do not "improve" or add sections.
- All colour, spacing, type and elevation values are tokens in
  `dashboard/css/tokens.css`. Never hardcode an ad-hoc value; never use default
  Tailwind palette colours.
- Financial semantics: gain/loss never by hue alone (always a sign and an arrow),
  tabular figures right-aligned, GBP as `£1,234.56`.
- Every data view needs loading, empty and stale states.
- WCAG AA in both themes, including P&L colours against their own tinted chip
  backgrounds.
- Screenshot loop: serve on localhost, screenshot to `dashboard/screenshots/`
  with auto-incremented names, read the image back, compare against `Design.pdf`,
  fix, re-screenshot. Minimum two rounds. Never screenshot a `file://` URL.
